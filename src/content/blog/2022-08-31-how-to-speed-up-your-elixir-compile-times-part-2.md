...
snippet: "Test your understanding of Elixir compile-time vs runtime dependencies with these practical challenges — then check your answers."
image:
  {
...
8. **Part a)** `a.ex` and `b.ex` would both be recompiled. When you use `import` then if any of the function signatures (i.e. names of functions or the number of arguments) change then `b.ex` needs to be recompiled.
   **Part b)** `a.ex` and `b.ex` would both be recompiled. Again because we used `import` any change in the function signatures (including adding a new function) causes recompilation.
...
In the next final article in this series I'll share strategies to help improve compile times.
